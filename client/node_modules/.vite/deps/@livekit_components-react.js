...
function setLogExtension(extension) {
  ...
  logger.methodFactory = (methodName, configLevel, loggerName) => {
    ...
    return (msg, context2) => {
      if (context2) rawMethod(msg, context2);
      else rawMethod(msg);
      if (needLog) {
...
var BinaryWriter = class {
   ...
   */
  sfixed64(value) {
    let chunk2 = new Uint8Array(8), view = new DataView(chunk2.buffer), tc = protoInt64.enc(value);
   ...
   */
  fixed64(value) {
    let chunk2 = new Uint8Array(8), view = new DataView(chunk2.buffer), tc = protoInt64.uEnc(value);
    view.setInt32(0, tc.lo, true);
    view.setInt32(4, tc.hi, true);
    return this.raw(chunk2);
  }
...
var DefaultReconnectPolicy = class {
  ...
  nextRetryDelayInMs(context2) {
    if (context2.retryCount >= this._retryDelays.length) return null;
    const retryDelay = this._retryDelays[context2.retryCount];
    if (context2.retryCount <= 1) return retryDelay;
...
var logging = log;
function shimGetUserMedia$2(window3, browserDetails2) {
  ...
  const constraintsToChrome_ = function(c) {
    ...
    Object.keys(c).forEach((key) => {
      ...
      const r3 = typeof c[key] === "object" ? c[key] : {
        ideal: c[key]
      };
...
function shimGetDisplayMedia$1(window3, getSourceId) {
...
function shimOnTrack$1(window3) {
...
function shimAddTrackRemoveTrack(window3, browserDetails2) {
  if (!window3.RTCPeerConnection) {
    return;
  }
  if (window3.RTCPeerConnection.prototype.addTrack && browserDetails2.version >= 65) {
  ...
  const origLocalDescription = Object.getOwnPropertyDescriptor(window3.RTCPeerConnection.prototype, "localDescription");
  ...
  window3.RTCPeerConnection.prototype.removeTrack = function removeTrack(sender) {
    ...
    const isLocal2 = sender._pc === this;
    if (!isLocal2) {
...
function shimPeerConnection$1(window3, browserDetails2) {
...
function fixNegotiationNeeded(window3, browserDetails2) {
...
function shimGetUserMedia$1(window3, browserDetails2) {
  const navigator2 = window3 && window3.navigator;
...
function shimPeerConnection(window3, browserDetails2) {
...
function shimMaxMessageSize(window3, browserDetails2) {
  if (!window3.RTCPeerConnection) {
    return;
  }
...
function removeExtmapAllowMixed(window3, browserDetails2) {
  if (!window3.RTCPeerConnection) {
    return;
  }
...
function shimAddIceCandidateNullOrEmpty(window3, browserDetails2) {
...
function shimParameterlessSetLocalDescription(window3, browserDetails2) {
  if (!(window3.RTCPeerConnection && window3.RTCPeerConnection.prototype)) {
    return;
  }
...
function adapterFactory() {
  ...
  const logging2 = log;
  const browserDetails2 = detectBrowser(window3);
  const adapter = {
  ...
  switch (browserDetails2.browser) {
    case "chrome":
      ...
      shimAddIceCandidateNullOrEmpty(window3, browserDetails2);
      shimParameterlessSetLocalDescription(window3);
      shimGetUserMedia$2(window3, browserDetails2);
      shimMediaStream(window3);
      shimPeerConnection$1(window3, browserDetails2);
      shimOnTrack$1(window3);
      shimAddTrackRemoveTrack(window3, browserDetails2);
      shimGetSendersWithDtmf(window3);
      shimGetStats(window3);
      shimSenderReceiverGetStats(window3);
      fixNegotiationNeeded(window3, browserDetails2);
      shimRTCIceCandidate(window3);
      shimRTCIceCandidateRelayProtocol(window3);
      shimConnectionState(window3);
      shimMaxMessageSize(window3, browserDetails2);
      shimSendThrowTypeError(window3);
      removeExtmapAllowMixed(window3, browserDetails2);
      break;
    case "firefox":
      ...
      shimAddIceCandidateNullOrEmpty(window3, browserDetails2);
      shimParameterlessSetLocalDescription(window3);
      shimGetUserMedia$1(window3, browserDetails2);
      shimPeerConnection(window3, browserDetails2);
      shimOnTrack(window3);
      shimRemoveStream(window3);
      shimSenderGetStats(window3);
      shimReceiverGetStats(window3);
      shimRTCDataChannel(window3);
      shimAddTransceiver(window3);
      shimGetParameters(window3);
      shimCreateOffer(window3);
      shimCreateAnswer(window3);
      shimRTCIceCandidate(window3);
      shimConnectionState(window3);
      shimMaxMessageSize(window3, browserDetails2);
      shimSendThrowTypeError(window3);
    ...
    case "safari":
      ...
      shimAddIceCandidateNullOrEmpty(window3, browserDetails2);
      shimParameterlessSetLocalDescription(window3);
      shimRTCIceServerUrls(window3);
      shimCreateOfferLegacy(window3);
      shimCallbacksAPI(window3);
      shimLocalStreamsAPI(window3);
      shimRemoteStreamsAPI(window3);
      shimTrackEventTransceiver(window3);
      shimGetUserMedia(window3);
      shimAudioContext(window3);
      shimRTCIceCandidate(window3);
      shimRTCIceCandidateRelayProtocol(window3);
      shimMaxMessageSize(window3, browserDetails2);
      shimSendThrowTypeError(window3);
...
var PCTransportManager = class {
  ...
  updateConfiguration(config2, iceRestart) {
    ...
    this.subscriber.setConfiguration(config2);
    if (iceRestart) {
  ...
  constructor(options) {
    ...
    this.handleDataMessage = (message) => __awaiter(this, void 0, void 0, function* () {
      ...
      try {
        ...
        const dp = DataPacket.fromBinary(new Uint8Array(buffer2));
...
function encodingsFromPresets(width, height, presets) {
  ...
  presets.forEach((preset, idx) => {
    ...
    const encoding = {
      rid,
      scaleResolutionDownBy: Math.max(1, size2 / Math.min(preset.width, preset.height)),
...
function setPublishingLayersForSender(sender, senderEncodings, qualities, senderLock, log3, logContext) {
  return __awaiter(this, void 0, void 0, function* () {
    const unlock = yield senderLock.lock();
    log3.debug("setPublishingLayersForSender", Object.assign(Object.assign({}, logContext), {
      sender,
      qualities,
      senderEncodings
    }));
    try {
      ...
      if (encodings.length !== senderEncodings.length) {
        log3.warn("cannot set publishing layers, encodings mismatch");
        return;
      }
      ...
      if (closableSpatial && encodings[0].scalabilityMode) ;
        ...
        encodings.forEach((encoding, idx) => {
          ...
          if (encoding.active !== subscribedQuality.enabled) {
            ...
            log3.debug("setting layer ".concat(subscribedQuality.quality, " to ").concat(encoding.active ? "enabled" : "disabled"), logContext);
            if (isFireFox()) {
      ...
      if (hasChanged) {
        ...
        log3.debug("setting encodings", Object.assign(Object.assign({}, logContext), {
...
var Participant = class extends eventsExports.EventEmitter {
  ...
  /** @internal */
  constructor(sid, identity2, name, metadata, loggerOptions) {
    var _a;
    super();
    this.audioLevel = 0;
...
var RemoteParticipant = class _RemoteParticipant extends Participant {
  ...
  /** @internal */
  constructor(signalClient, sid, identity2, name, metadata, loggerOptions) {
    super(sid, identity2 || "", name, metadata, loggerOptions);
    this.signalClient = signalClient;
    this.tracks = /* @__PURE__ */ new Map();
    this.audioTracks = /* @__PURE__ */ new Map();
    this.videoTracks = /* @__PURE__ */ new Map();
...
var LocalParticipant = class extends Participant {
  ...
  constructor(sid, identity2, engine, options) {
    super(sid, identity2, void 0, void 0, {
      loggerName: options.loggerName,
...
function tokenize(input, grammar2) {
  const matches = Object.entries(grammar2).map(
...
var log2 = import_loglevel.default.getLogger("lk-components-js");
log2.setDefaultLevel("WARN");
...
function setLogExtension2(extension, options = {}) {
  ...
  log2.methodFactory = (methodName, configLevel, loggerName) => {
    const rawMethod = originalFactory(methodName, configLevel, loggerName);
    const logLevel = LogLevel[methodName];
    const needLog = logLevel >= configLevel && logLevel < LogLevel.silent;
    return (msg, context2) => {
...
function observeRoomEvents(room, ...events2) {
...
function roomInfoObserver(room) {
  const observer = observeRoomEvents(
    ...
    map((r3) => {
      return { name: r3.name, metadata: r3.metadata };
    })
  );
  return observer;
}
...
function createMediaDeviceObserver(kind, onError, requestPermissions = true) {
  ...
  const onDeviceChange = () => __async(this, null, function* () {
    try {
      const newDevices = yield Room2.getLocalDevices(kind, requestPermissions);
  ...
  return concat(
    Room2.getLocalDevices(kind, requestPermissions).catch((e) => onError == null ? void 0 : onError(e)),
    observable2
  );
}
...
function createActiveDeviceObservable(room, kind) {
  return roomEventSelector(room, RoomEvent.ActiveDeviceChanged).pipe(
    ...
    map(([kind2, deviceId]) => {
      log2.debug("activeDeviceObservable | RoomEvent.ActiveDeviceChanged", { kind: kind2, deviceId });
      return deviceId;
    }),
...
function observeParticipantEvents(participant, ...events2) {
  const observable2 = new Observable((subscribe) => {
...
function connectedParticipantsObserver(room, options = {}) {
  ...
  const listener = observeRoomEvents(room, ...roomEvents).subscribe(
    (r3) => subscriber == null ? void 0 : subscriber.next(Array.from(r3.participants.values()))
  );
...
function connectedParticipantObserver(room, identity2, options = {}) {
  ...
  const observable2 = observeRoomEvents(
    ...
    switchMap((r3) => {
      const participant = r3.getParticipantByIdentity(identity2);
      if (participant) {
...
function setupDeviceSelector(kind, room, localTrack) {
  ...
  const setActiveMediaDevice = (_0, ..._1) => __async(this, [_0, ..._1], function* (id, options = {}) {
    var _a, _b, _c;
    if (room) {
      log2.debug(`Switching active device of kind "${kind}" with id ${id}.`);
...
function setupStartAudio() {
  const handleStartAudioPlayback = (room) => __async(this, null, function* () {
    log2.info("Start Audio for room: ", room);
...
function setupStartVideo() {
  const handleStartVideoPlayback = (room) => __async(this, null, function* () {
    log2.info("Start Video for room: ", room);
...
function trackReferencesObservable(room, sources, options) {
  ...
  const observable2 = observeRoomEvents(room, ...roomEvents).pipe(
    map((room2) => {
      ...
      log2.debug(`TrackReference[] was updated. (length ${data.trackReferences.length})`, data);
      return data;
    }),
...
function saveToLocalStorage(key, value) {
  if (typeof localStorage === "undefined") {
    log2.error("Local storage is not available.");
    return;
  }
  try {
    localStorage.setItem(key, JSON.stringify(value));
  } catch (error) {
    log2.error(`Error setting item to local storage: ${error}`);
  }
}
function loadFromLocalStorage(key) {
  ...
  try {
    ...
    if (!item) {
      log2.warn(`Item with key ${key} does not exist in local storage.`);
      return void 0;
    }
    return JSON.parse(item);
  } catch (error) {
    log2.error(`Error getting item from local storage: ${error}`);
    return void 0;
  }
}
...
function loadUserChoices(defaults, preventLoad = false) {
  ...
  if (preventLoad) {
    ...
    const result = __spreadValues(__spreadValues({}, fallback2), maybeLoadedObject != null ? maybeLoadedObject : {});
    return result;
  }
}
...
function useResizeObserver(target, callback) {
  const resizeObserver2 = getResizeObserver2();
  ...
  React2.useLayoutEffect(() => {
    ...
    resizeObserver2 == null ? void 0 : resizeObserver2.subscribe(targetEl, cb);
    return () => {
      didUnsubscribe = true;
      resizeObserver2 == null ? void 0 : resizeObserver2.unsubscribe(targetEl, cb);
    };
  }, [target.current, resizeObserver2, storedCallback]);
  return resizeObserver2 == null ? void 0 : resizeObserver2.observer;
}
...
var _resizeObserver;
var getResizeObserver2 = () => !_resizeObserver ? _resizeObserver = createResizeObserver() : _resizeObserver;
var useSize = (target) => {
...
function useEnsureRoom(room) {
  ...
  const r3 = room != null ? room : context2;
  if (!r3) {
    throw new Error(
...
function useConnectionState(room) {
  const r3 = useEnsureRoom(room);
  const observable2 = React12.useMemo(() => connectionStateObserver(r3), [r3]);
  const connectionState = useObservableState(observable2, r3.state);
...
function useLiveKitRoom(props) {
  ...
  React19.useEffect(() => {
    setRoom(passedRoom != null ? passedRoom : new Room2(options));
  }, [passedRoom]);
  ...
  React19.useEffect(() => {
    ...
    const onSignalConnected = () => {
      ...
      log2.debug("trying to publish local tracks");
      Promise.all([
  ...
  React19.useEffect(() => {
    ...
    if (!token) {
      log2.debug("no token yet");
      return;
    }
    if (!serverUrl) {
      log2.warn("no livekit url provided");
      onError == null ? void 0 : onError(Error("no livekit url provided"));
      return;
    }
    if (connect2) {
      log2.debug("connecting");
      room.connect(serverUrl, token, connectOptions).catch((e) => {
        log2.warn(e);
        onError == null ? void 0 : onError(e);
      });
    } else {
      log2.debug("disconnecting because connect is false");
      room.disconnect();
    }
  }, [
    connect2,
  ...
  React19.useEffect(() => {
    if (!room)
      return;
    return () => {
      log2.info("disconnecting on onmount");
      room.disconnect();
    };
  }, [room]);
...
function useTracks(sources = [
  ...
  React44.useEffect(() => {
    const subscription = trackReferencesObservable(room, sources_, {
      ...
      log2.debug("setting track bundles", trackReferences2, participants2);
...
function AudioTrack(_a) {
  ...
  React82.useEffect(() => {
    ...
    if (track instanceof RemoteAudioTrack) {
      ...
      log2.warn("Volume can only be set on remote audio tracks.");
    }
  }, [volume, track]);
  React82.useEffect(() => {
    ...
    if (pub instanceof RemoteTrackPublication) {
      ...
      log2.warn("Can only call setEnabled on remote track publications.");
    }
...
function LayoutContextProvider({
  ...
  React90.useEffect(() => {
    log2.debug("PinState Updated", { state: layoutContextValue.pin.state });
  ...
  React90.useEffect(() => {
    log2.debug("Widget Updated", { widgetState: layoutContextValue.widget.state });
...
function usePreviewDevice(enabled, deviceId, kind) {
  ...
  React103.useEffect(() => {
    if (enabled && !localTrack && !deviceError && !isCreatingTrack) {
      log2.debug("creating track", kind);
  ...
  React103.useEffect(() => {
    ...
    if (!enabled) {
      log2.debug(`muting ${kind} track`);
      localTrack.mute().then(() => log2.debug(localTrack.mediaStreamTrack));
      ...
      log2.debug(`switching ${kind} device from`, prevDeviceId.current, selectedDevice.deviceId);
      switchDevice(localTrack, selectedDevice.deviceId);
    } else {
      log2.debug(`unmuting local ${kind} track`);
      localTrack.unmute();
    }
  ...
  React103.useEffect(() => {
    return () => {
      if (localTrack) {
        log2.debug(`stopping local ${kind} track`);
        localTrack.stop();
        localTrack.mute();
      }
    };
...
function PreJoin(_a) {
  ...
  function handleSubmit(event) {
    ...
    if (handleValidation(userChoices)) {
      ...
      log2.warn("Validation failed with: ", userChoices);
    }
  }
...
function VideoConference(_a) {
  ...
  const widgetUpdate = (state) => {
    log2.debug("updating widget state", state);
  ...
  React106.useEffect(() => {
    ...
    if (screenShareTracks.some((track) => track.publication.isSubscribed) && lastAutoFocusedScreenShareTrack.current === null) {
      log2.debug("Auto set screen share focus:", { newScreenShareTrack: screenShareTracks[0] });
      ...
      log2.debug("Auto clearing screen share focus.");
  ...
  useWarnAboutMissingStyles();
  return React106.createElement("div", __spreadValues2({ className: "lk-video-conference" }, props), isWeb2() && React106.createElement(
